...
full_join(cr, members) |> select(data_row_id, match_id, bioname, icpsr, congress, state, district_code, url) |> kable()
```


- [ ] integrate with `congress` and/or `congressData`. For example, `augmentCongress` may simply join in identifiers for other datasets on ICPSR numbers. Perhaps this is best left to `congress`. 
