...
[`extractMemberName()`](reference/extractMemberName.md).
...
The main function,
[`extractMemberName()`](reference/extractMemberName.md), returns a
dataframe of the names and ICPSR ID numbers of members of Congress in a
supplied vector of text.

> in the future, [`extractMemberName()`](reference/extractMemberName.md)
> may default to returning a list of dataframes the same length as the
> supplied data

For example, we can use
[`extractMemberName()`](reference/extractMemberName.md) to detect the
names of members of Congress in the text of the Congressional Record.
Let’s start with text from the Congressional Record from 3/1/2007,
...
[`extractMemberName()`](reference/extractMemberName.md) can also search
much longer and messier texts, including text where names are not
...
Because [`extractMemberName()`](reference/extractMemberName.md) links
each detected name to ICPSR IDs from voteview.com, we already have some
information, like state and district for each legislator detected in the
...
[`extractMemberName()`](reference/extractMemberName.md) needs options to
supply custom typos and additions to the main regex table
