...
members[c("chamber", "congress", "bioname", "pattern")]  |> kable()
```
...
full_join(cr, members) |> select(data_row_id, match_id, bioname, icpsr, congress, state, district_code, party_name, nominate.dim1, url) |> kable()
```


- [ ] integrate with `congress` and/or `congressData`. For example, we may want a function, `augmentCongress` to simply join in identifiers for other datasets on ICPSR numbers. Perhaps this is best left to `congress`. 
